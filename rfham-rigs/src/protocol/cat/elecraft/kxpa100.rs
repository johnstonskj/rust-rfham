...
define_command!("Get the amplifier's ADC readings (PA drain voltage, drain current, and supply
voltage).
...
define_command!("Get whether auto-bias is enabled.
...
define_command!("Set whether auto-bias is enabled.
...
define_command!("Get the selected antenna port.
...
define_command!("Set the antenna port.
...
define_command!("Get whether the internal ATU is switched into circuit.
...
define_command!("Set whether the internal ATU is switched into circuit.
...
define_command!("Get the PC-side RS-232 port data rate.
...
define_command!("Set the PC-side RS-232 port data rate.
...
define_command!("Get the transceiver-side RS-232 port data rate.
...
define_command!("Set the transceiver-side RS-232 port data rate.
...
define_command!("Get the busy and PTT status.
...
define_command!("Reset the amplifier configuration to factory defaults.
...
define_command!("Get the accumulated error count since the last reset.
...
define_command!("Get the most recent error message.
...
define_command!("Get the operating frequency currently reported to the amplifier.
...
define_command!("Inform the amplifier of the operating frequency, in Hz.
...
define_command!("Get the frequency entry source.
...
define_command!("Set the frequency entry source.
...
define_command!("Get the fan-on threshold temperature.
...
define_command!("Set the fan-on threshold temperature.
...
define_command!("Get the PA drain current.
...
define_command!("Get the low-pass filter relay state.
...
define_command!("Set the low-pass filter relay state.
...
define_command!("Get the current operating mode.
...
define_command!("Set the operating mode.
...
define_command!("Get the front-panel meter display selection.
...
define_command!("Set the front-panel meter display selection.
...
define_command!("Get the current RF output power.
...
define_command!("Get the peak-power control setting.
...
define_command!("Set the peak-power control setting.
...
define_command!("Get whether protection-fault detection is enabled.
...
define_command!("Set whether protection-fault detection is enabled.
...
define_command!("Get the DC supply input voltage.
...
define_command!("Get the PA drain voltage.
...
define_command!("Get the RF sense level detected at the amplifier input.
...
define_command!("Get the SWR protection-inhibit threshold.
...
define_command!("Set the SWR protection-inhibit threshold.
...
define_command!("Get the current SWR reading.
...
define_command!("Get the amplifier serial number.
...
define_command!("Get the DC supply voltage.
...
define_command!("Get whether SWR fault protection is enabled.
...
define_command!("Set whether SWR fault protection is enabled.
...
define_command!("Get the PA heatsink temperature.
...
define_command!("Get the RF power level used during an ATU tune cycle.
...
define_command!("Set the RF power level used during an ATU tune cycle.
...
define_command!("Initiate an ATU tune cycle.
...
define_command!("Get the radio interface type and option bit.
...
define_command!("Get the transceiver drive power level detected by the amplifier.
...
impl_command!(GetAutoBiasEnable => b"^AE");
impl_command_with_response!(GetAutoBiasEnable => boolean);

impl_command!(SetAutoBiasEnable => b"^AE" for boolean enabled);
...
impl_command!(GetAtuEnable => b"^AT");
impl_command_with_response!(GetAtuEnable => boolean);

impl_command!(SetAtuEnable => b"^AT" for boolean enabled);
...
impl_command!(GetBusyStatus => b"^BY");
impl_command_with_response!(GetBusyStatus => 2, |bytes: &[u8]| {
    Ok(BusyStatus {
        busy: bytes[0] == b'1',
...
impl_command!(ResetConfiguration => b"^CR");
...
impl_command!(GetErrorCount => b"^EC");
impl_command_with_response!(GetErrorCount => 4, u16_from_ascii => u16);
...
impl_command!(GetFrequencyEntryMode => b"^FE");
impl_command_with_response!(GetFrequencyEntryMode => boolean);

impl_command!(SetFrequencyEntryMode => b"^FE" for boolean manual);
...
impl_command!(GetFanThreshold => b"^FT");
impl_command_with_response!(GetFanThreshold => 3, u8_from_ascii => u8);

impl_command!(SetFanThreshold => b"^FT" format celsius uint 3);

// ------------------------------------------------------------------------------------------------

impl_command!(GetDrainCurrent => b"^I");
impl_command_with_response!(GetDrainCurrent => 3, u16_from_ascii => u16);

// ------------------------------------------------------------------------------------------------

impl_command!(GetLowPassRelay => b"^LR");
impl_command_with_response!(GetLowPassRelay => boolean);

impl_command!(SetLowPassRelay => b"^LR" for boolean in_line);

// ------------------------------------------------------------------------------------------------

impl_command!(GetOperatingMode => b"^MD");
impl_command_with_response!(GetOperatingMode => boolean);

impl_command!(SetOperatingMode => b"^MD" for boolean operate);

// ------------------------------------------------------------------------------------------------

impl_command!(GetMeterDisplay => b"^MT");
impl_command_with_response!(GetMeterDisplay => 1, u8_from_ascii => u8);
...
impl_command_with_response!(GetOutputPower => 3, u8_from_ascii => u8);

// ------------------------------------------------------------------------------------------------

impl_command!(GetPeakPowerControl => b"^PC");
impl_command_with_response!(GetPeakPowerControl => 3, u8_from_ascii => u8);
...
impl_command!(GetPowerInput => b"^PI");
impl_command_with_response!(GetPowerInput => 3, u16_from_ascii => u16);

// ------------------------------------------------------------------------------------------------

impl_command!(GetPaVoltage => b"^PV");
impl_command_with_response!(GetPaVoltage => 3, u16_from_ascii => u16);

// ------------------------------------------------------------------------------------------------

impl_command!(GetRfSense => b"^RS");
impl_command_with_response!(GetRfSense => 3, u8_from_ascii => u8);

// ------------------------------------------------------------------------------------------------

impl_command!(GetFirmwareVersion => b"^RV");
impl_command_with_response!(GetFirmwareVersion => 5, bytes_to_vec => Vec<u8>);

// ------------------------------------------------------------------------------------------------

impl_command!(GetSwrInhibitThreshold => b"^SI");
impl_command_with_response!(GetSwrInhibitThreshold => 2, u8_from_ascii => u8);

impl_command!(SetSwrInhibitThreshold => b"^SI" format swr_d uint 2);

// ------------------------------------------------------------------------------------------------

impl_command!(GetSwrMeter => b"^SM");
impl_command_with_response!(GetSwrMeter => 2, u8_from_ascii => u8);
...
impl_command!(GetSupplyVoltage => b"^SV");
impl_command_with_response!(GetSupplyVoltage => 3, u16_from_ascii => u16);

// ------------------------------------------------------------------------------------------------

impl_command!(GetSwrFaultEnable => b"^SW");
impl_command_with_response!(GetSwrFaultEnable => boolean);

impl_command!(SetSwrFaultEnable => b"^SW" for boolean enabled);
...
impl_command_with_response!(GetTunePower => 3, u8_from_ascii => u8);

impl_command!(SetTunePower => b"^TP" format watts uint 3);

// ------------------------------------------------------------------------------------------------

impl_command!(InitiateTune => b"^TU");
...
impl_command!(GetTransceiverPowerLevel => b"^XP");
impl_command_with_response!(GetTransceiverPowerLevel => 3, u8_from_ascii => u8);
